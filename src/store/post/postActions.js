...
export const createPost = createAsyncThunk('post/createPost', async (data, thunkAPI) => {
  ...
  if (data.files.length > 0) {
    ...
    const uploadTask = thunkAPI.dispatch(uploadFiles({ files, documentId }));
...
export const deleteDocument = createAsyncThunk(
  'post/deleteDocument',
  async ({ postId, documentId, index, filename }) => {
...
export const updateComment = createAsyncThunk('post/updateComment', async (commentData) => {
  // Extract the necessary data from commentData
  const { postId, commentId, commentValue } = commentData;

  const docRef = doc(db, 'posts', postId);
  const commentForChange = await getDoc(docRef).then((doc) => doc.data().comments);
  let updatedComments = [];
...
export const fetchPosts = createAsyncThunk('post/fetchPosts', async () => {
  const postsCollectionRef = query(collection(db, 'posts'), orderBy('createdAt', 'desc'));
